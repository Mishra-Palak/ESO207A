...
int selectX(int *A, int i, int N) {
    ...
    if(n_meds > 1) {
        median_of_medians = selectX(A, n_meds/2, n_meds);
    } else {
    ...
    if(k == i) {
	    ...
	    return selectX(A, i, k);
    } else {
        return selectX(A+k, i-k, N-k);
    }
}
...
void multiplyX(long long int** A, long long int** B, int N, long long int** C) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            C[i][j] = 0;
            for (int k = 0; k < N; k++) {
...
void matrix() {
    ...
    for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) fscanf(random, "%lld", &A[i][j]);
    fclose(random);
    ...
    for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) fscanf(random2, "%lld", &B[i][j]);
    fclose(random2);
