...
void mergeVsHeap() {
    ...
    for (int i = 0; i < 100; i++) {
        ...
        t1 = clock();
        mergeSort(B, 0, N-1); //Best - O(nlgn), Avg - O(nlgn), Worst - O(nlgn)
        t1 = clock()-t1;
        ...
        t2 = clock();
        heapSort(C, N); //Best - O(nlgn), Avg - O(nlgn), Worst - O(nlgn)
        t2 = clock()-t2;
    ...
    printf("\n");
    printf("Average Time Taken by Merge Sort: %lf\n", t1_avg);
    printf("Average Time Taken by Heap Sort: %lf\n", t2_avg);
    if (t1_avg < t2_avg) printf("Merge Sort is faster in case of N = %d.\n", N);
    else if (t1_avg == t2_avg) printf("Time taken by both the sorts are comparable in case of N = %d.\n", N);
    else printf("Heap Sort is faster in case of N = %d.\n", N);
}

//MatrixMultiplication
void multiplyX(int** A, int** B, int N, int** C) {
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            C[i][j] = 0;
            for (int k = 0; k < N; k++) {
...
void multiply(int **A, int** B, int N, int n2, int** C) {
    if (N == 1) {
        C[0][0] = A[0][0]*B[0][0];
    } else {
        int n = N/2;
        int **A11 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            A11[i]= malloc(n*sizeof(int));
        }
        int **A12 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            A12[i]= malloc(n*sizeof(int));
        }
        int **A21 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            A21[i]= malloc(n*sizeof(int));
        }
        int **A22 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            A22[i]= malloc(n*sizeof(int));
        }
        int **B11 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            B11[i]= malloc(n*sizeof(int));
        }
        int **B12 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            B12[i]= malloc(n*sizeof(int));
        }
        int **B21 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            B21[i]= malloc(n*sizeof(int));
        }
        int **B22 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            B22[i]= malloc(n*sizeof(int));
        }
        ...
        for(int i = 0; i < n; i++){
            temp1[i] = malloc(n*sizeof(int));
        }
        int **temp2 = malloc(n*sizeof(int *));
        for(int i = 0; i < n; i++){
            temp2[i] = malloc(n*sizeof(int));
        }
        sub(A12, A22, n, temp1);
        add(B21, B22, n, temp2);
        ...
        int **temp3 = malloc(n*sizeof(int *));
        for(int i = 0; i < n; i++){
            temp3[i] = malloc(n*sizeof(int));
        }
        int **temp4 = malloc(n*sizeof(int *));
        for(int i = 0; i < n; i++){
            temp4[i] = malloc(n*sizeof(int));
        }
        add(A11, A22, n, temp3);
        add(B11, B22, n, temp4);
        ...
        int **temp5 = malloc(n*sizeof(int *));
        for(int i = 0; i < n; i++){
            temp5[i] = malloc(n*sizeof(int));
        }
        int **temp6 = malloc(n*sizeof(int *));
        for(int i = 0; i < n; i++){
            temp6[i] = malloc(n*sizeof(int));
        }
        sub(A11, A21, n, temp5);
        add(B11, B12, n, temp6);
        ...
        int **temp7 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            temp7[i]= malloc(n*sizeof(int));
        }       
        add(A11, A12, n, temp7);
        multiply(temp7, B22, n, N, S);
        free(temp7);

        int **temp8 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            temp8[i]= malloc(n*sizeof(int));
        }       
        add(A21, A22, n, temp8);
        multiply(temp8, B11, n, N, T);
        free(temp8);

        int **temp9 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            temp9[i]= malloc(n*sizeof(int));
        }       
        sub(B12, B22, n, temp9);
        multiply(A11, temp9, n, N, U);
        free(temp9);

        int **temp10 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            temp10[i]= malloc(n*sizeof(int));
        }       
        ...
        int **t4 = malloc(n * sizeof(int *));
        for(int i = 0; i < n; i++){
            t4[i]= malloc(n*sizeof(int));
        }  
...
void matrix() {
    ...
    for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) fscanf(random, "%d", &A[i][j]);
    fclose(random);

    randomNumberGenerator(N*N, 2);
    FILE * random2 = fopen("random.txt", "r");
    int **B = malloc(N * sizeof(int *));
    for(int i = 0; i < N; i++){
        B[i]= malloc(N*sizeof(int));
    }
    for (int i = 0; i < N; i++) for (int j = 0; j < N; j++) fscanf(random2, "%d", &B[i][j]);
    fclose(random2);
