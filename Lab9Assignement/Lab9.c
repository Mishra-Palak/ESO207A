...
tree_node* search(red_black_tree *t, int key) {
    tree_node *y = t->root;
    while (y != t->NIL && key != y->data) {
        if (key < y->data) y = y->left;
        else y = y->right;
    }
    return y;
}
...
int main() {
    ...
    while(choice != 5) {
        if (choice == 1) {
            ...
            scanf("%d", &item);
            a = new_tree_node(item);
            insert(t, a);
        } else if (choice == 2) {
            int item;
            printf("Enter the item: ");
            scanf("%d", &item);
            tree_node* a = search(t, item);
            if (a->data == item) {
                printf("Item found.\n");
            }
    ...
    printf("\n-----SECOND TASK-----\n");
    red_black_tree *r = new_red_black_tree();
    randomNumberGenerator(500);

    FILE * random = fopen("random.txt", "r");
    for (int i = 0; i < 500; i++) {
        int item;
        fscanf(random, "%d", &item);
        tree_node *a = new_tree_node(item);
        insert(r, a);
    }
    fclose(random);
